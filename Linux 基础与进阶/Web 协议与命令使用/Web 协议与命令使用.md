...
- [🌐 Web 协议与命令使用](#-web-协议与命令使用)
  - [文档目录](#文档目录)
  - [Cookie 与 Session 的区别及联系](#cookie-与-session-的区别及联系)
  - [wget 命令使用](#wget-命令使用)
  - [curl 命令使用](#curl-命令使用)

1. HTTP 的协议版本和区别？
2. HTTP 新版本是否完全兼容旧版本吗？
3. 现代服务器的分层降级：

```plaintext
用户请求
   ↓
[QUIC/HTTP/3] 尝试 UDP 443
   ↓ 失败
[TCP + TLS 1.3] ALPN 协商 h2
   ↓ 失败
[TCP + TLS] ALPN 协商 http/1.1
   ↓ 失败
[TCP] 明文 HTTP/1.1（已不推荐）
```

4. Nginx 服务端是否支持不同的 HTTP 版本? 该如何去部署实现呢? 浏览器客户端本身使用哪个 HTTP 协议版本呢？
...
└──────┬──────┘     └─────────────────────────────────────────┘
       │                                                         
       │  1. DNS 查询（检查 HTTPS RR 记录）                       
       │ ───────────────────────────────────────►                
       │ ◄─────────────────────────────────────── 返回 h3 支持信息 
       │                                                         
       │  2. 尝试 QUIC/HTTP/3（UDP 443）                          
       │ ═══════════════════════════════════════► 成功 → 使用 h3   
       │  失败 ─┐                                                 
       │        │  3. TCP + TLS 握手                              
       │        └──────────────────────────────►                  
       │           ClientHello 携带 ALPN: [h2, http/1.1]          
       │ ◄─────────────────────────────────────── ServerHello     
       │           返回选定协议（如 h2）                            
       │                                                         
       │  4. 应用层通信（HTTP/2 或 HTTP/1.1）                       
       │ ───────────────────────────────────────►                
```

5. 什么是 ALPN 协商？
6. SSL 与 TLS 的关系和区别是什么？
...
- 常用选项：
  ...
  <img src="images/curl-k-https.jpg" width=70%>
