...
- [🛡️ 一文厘清 HTTPS 原理与应用 (RHEL7.x)](#️-一文厘清-https-原理与应用-rhel7x)
  - [文档说明](#文档说明)
  - [文档目录](#文档目录)
  - [加密通信背景](#加密通信背景)
...
- 客户端验证证书过程：
  ...
  - 🚀 验证过程与原理，如下所示：
    ...
    <center><img src="images-7/ca-signed-certification-verify.jpg" style="width:60%"></center>
...
- SSL/TLS 协议在网络模型中的位置：
  ...
  <center><img src="images-7/ssl-tls-in-network-stack.png" style="width:60%"></center>
...
- 服务端本地与客户端本地的 SSL 套接字与 TCP 套接字的关系，如下所示：
  ...
  <center><img src="images-7/client-server-tcp-ssl-socket.png" style="width:60%"></center>
...
- ✨ 以下从 HTTPS 抓包的角度说明 SSL/TLS 四次握手与 HTTPS 单/双向认证的详细过程：
  ...
  <center><img src="images-7/ssl-four-handshakes-https-single-and-mutual-authentication.png" style="width:60%"></center>
...
- Nginx 容器使用的 [配置文件](https://github.com/Alberthua-Perl/dockerfile-s2i-demo/blob/master/nginx-ssl/www.conf)，如下所示：
  ✨ 该配置文件启用 HTTPS 单向认证，如果需开启双向认证过程，只需启用 `ssl_client_certificate` 与 `ssl_verify_client` 参数即可。
...
- HTTPS 加密通信 - 抓包整体示意：
  ...
  ![wireshark-https-single-progress](images-7/wireshark-https-single-progress.png)

- 🤝 HTTPS 加密通信 - 4 次握手过程示意：
  
  ![ssl-tls-single-authentication-progress](images-7/ssl-tls-single-authentication-progress.png)
...
- 1️⃣ HTTPS 加密通信 - 第 1 次握手过程：`Client Hello`
  ...
  - Client Hello 握手信息中包含如下内容：
    ...
    - 扩展字段 Extensions，支持协议与算法的相关参数以及其它辅助信息等，常见的 SNI 就属于扩展字段。
      ...
      ![client-hello-body-1](images-7/client-hello-body-1.png)
  
  - 客户端支持的 17 种加密套件供服务端选择使用。

    ![client-hello-body-2](images-7/client-hello-body-2.png)

- 2️⃣ HTTPS 加密通信 - 第 2 次握手过程：服务端给客户端回复的 4 条 SSL 握手信息
  
  ![server-response-to-client-ssl](images-7/server-response-to-client-ssl.png)
  ...
  - `Certificate`：该 SSL 握手信息中包含服务端 CA 数字签名证书
    ...
    ![server-ca-signed-certification](images-7/server-ca-signed-certification.png)
  
  - `Server Key Exchange`：

    使用 `EC Diffie-Hellman` 算法（`ECDHE`）实现服务端与客户端的密钥交换算法协商。

    ![server-key-exchange](images-7/server-key-exchange.png)
  ...
  - `Server Hello Done`：通知客户端 Server Hello 信息发送结束
    ...
    ![server-hello-done](images-7/server-hello-done.jpg)

- 3️⃣ HTTPS 加密通信 - 第 3 次握手过程：客户端给服务端回复 3 条 SSL 握手信息
  
  ![client-response-to-server-ssl](images-7/client-response-to-server-ssl.jpg)
...
- 4️⃣ HTTPS 加密通信 - 第 4 次握手过程：服务端给客户端回复 2 条 SSL 握手信息
  ...
  ![server-response-to-client-4-phase](images-7/server-response-to-client-4-phase.png)
...
- HTTPS 加密通信 - 握手结束：
  ...
  - 开始使用会话密钥与加密算法进行加密通信。
    ...
    ![ssl-tls-handshake-end](images-7/ssl-tls-handshake-end.png)

## HTTPS 单向认证测试

- 服务端启用 HTTPS 单向认证后，可从浏览器客户端进行访问测试：
  
  ![https-single-auth-chrome-error-1](images-7/https-single-auth-chrome-error-1.png)

- 该服务端 CA 数字签名使用未经认证的 CA 签发，因此客户端浏览器无法验证其安全性而发出警告，可点击 "高级" 按钮接受该证书继续访问。如果拒绝该证书，即断开此次认证连接，可在如下 Wireshark 抓包中显示安全告警信息：
  
  ![https-single-auth-chrome-error-2](images-7/https-single-auth-chrome-error-2.png) 
...
- HTTPS 双向认证过程的客户端测试：
  ...
  - 如果将客户端数字签名证书与私钥用于 `Firefox` 或 `Chrome` 浏览器访问服务端，可将其转换为 `p12` 格式：
  ...
  - 将 p12 格式的文件导入 Firefox 浏览器客户端：
    ...
    ![firefox-import-pc12-certs-1](images-7/firefox-import-pc12-certs-1.png)

    ![firefox-import-pc12-certs-2](images-7/firefox-import-pc12-certs-2.png)

    ![firefox-import-pc12-certs-3](images-7/firefox-import-pc12-certs-3.png)

    ![firefox-import-pc12-certs-4](images-7/firefox-import-pc12-certs-4.png)
  
  - 打开 Firefox 浏览器访问服务端，此时需接受客户端证书来标记自己：

    ![firefox-import-pc12-certs-5](images-7/firefox-import-pc12-certs-5.png)
  
  - 💥 如果双向认证客户端配置错误，将无法正常访问服务端，并且浏览器直接返回如下信息，且 Wireshark 抓包显示 `Encrypted Alert`：

    ![https-mutual-no-client-cert-error-1](images-7/https-mutual-no-client-cert-error-1.png)

    ![https-mutual-no-client-cert-error-2](images-7/https-mutual-no-client-cert-error-2.png)
