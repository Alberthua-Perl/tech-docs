...
- 上游 `Origin` 项目与 `OpenShift` 项目的发展对应关系：
  ...
  ![ocp3-origion-developer](pictures/ocp3-origion-developer.jpg)
...
- OCP 3.x 集群架构：
  ...
  ![ocp3-arch](pictures/ocp3-arch.png)

- OCP 4.x 集群架构：
  
  ![ocp4-arch](pictures/ocp4-arch.jpg)
  ...
  ![kubernetes-service-in-ocp4](pictures/kubernetes-service-in-ocp4.jpg)
  
  ![openshift-service-in-ocp4](pictures/openshift-service-in-ocp4.jpg)
...
- OCP 4.x 集群网络拓扑示例：  
  ...
  ![ocp4-sourth-north-east-west-traffic](pictures/ocp4-sourth-north-east-west-traffic.png)
...
- OCP 3.x 集群部署方法：  
  ...
  - 如下所示，minikube 在 RHEL 8.0 中的安装报错：
    ...
    ![minikube-error-1](pictures/minikube-error-1.jpg)
    
    ![minikube-error-2](pictures/minikube-error-2.jpg)
    
    ![minikube-error-3](pictures/minikube-error-3.jpg)
...
- 集群登录：
  ...
  ![system-admin-logout](pictures/system-admin-logout.jpg)
  
  ![docker-registry-route](pictures/docker-registry-route.jpg)
...
- master 节点：
  ...
  - master 节点执行的服务包括：
    ...
    - `data-store`：
      ...
      ![ocp3-master-pod](pictures/ocp3-master-pod.jpg)
      
      ![ocp3-master-etcd](pictures/ocp3-master-etcd.jpg)
...
- compute 节点
  ...
  - 每个节点上的 `atomic-openshift-node` 服务已集成 `kubelet` 功能。
    ...
    ![atomic-openshift-node-error-1](pictures/atomic-openshift-node-error-1.jpg)  
    
    ![atomic-openshift-node-error-2](pictures/atomic-openshift-node-error-2.jpg)
...
- image stream（`is`）、image stream tag（`istag`）：
  ...
  - image stream tag 保存了从 OCP external registry 或 OCP internal registry 获取的最新 image ID 的历史记录。
    ...
    ![oc-describe-is](pictures/oc-describe-is.jpg)
  ...
  - 从 OCP external registry 导入外部容器镜像至 OCP 集群：
    ...
    - 方式 2：
      ...
      - oc import-image 命令创建 `image stream tag` 引用外部 `私有` 容器镜像。
        ...
        ![oc-import-image-private-is](pictures/oc-import-image-private-is.jpg)
    ...
    - 🚀 默认情况下，oc import-image 命令使用 `--reference-policy=source` 选项，创建 image stream tag 引用 OCP external registry，不将容器镜像缓存至 OCP internal registry 中，因此，删除外部私有镜像仓库后将无法构建应用镜像。
      ...
      - 未删除外部私有镜像时，可被正确拉取构建，追踪 BUILD_LOGLEVEL=4 的 build 日志，如下所示：
        ...
        ![project-imagestream-reference-external-private-image-exist](pictures/project-imagestream-reference-external-private-image-exist.jpg)
      
      - 删除外部私有镜像后，将无法从外部拉取，如下所示：
        
        ![project-imagestream-reference-external-private-image-not-exist](pictures/project-imagestream-reference-external-private-image-not-exist.jpg)
    ...
    - 🚀 而 `--reference-policy=local` 将 OCP external registry 中的容器镜像缓存至 OCP internal registry，相当于在集群本地实现了 `镜像缓存`，可加速应用构建，即使删除外部私有镜像后也不影响应用镜像的构建，如下所示：
      ...
      ![import-image-reference-policy-local-test](pictures/import-image-reference-policy-local-test.jpg)
      
      skopeo 命令验证是否存在镜像缓存：
      
      ![skopeo-internal-registry-cache-layer](pictures/skopeo-internal-registry-cache-layer.jpg)
  ...
  - 使用来自另一个项目的 image stream（基于 private registry）构建与部署应用：
    ...
    - 方式 2：
      ...
      ![oc-import-image-shared-is-between-project](pictures/oc-import-image-shared-is-between-project.jpg)
  ...
  - 💎 补充：OCP 4.x internal registry 概要
    ...
    - 使用集群管理员（`cluster-admin`）权限即可暴露 internal registry，对外暴露的路由信息可被集群外部访问，如下所示：
      ...
      ![ocp4-operator-oc-patch-internal-registry-expose-route](pictures/ocp4-operator-oc-patch-internal-registry-expose-route.jpg)
    
    - internal registry 位于 `openshift-image-registry` 项目中，以 `image-registry pod` 的形式运行，并可通过 `cluster-image-registry-operator` 控制。
      
      ![ocp4-internal-registry-operator](pictures/ocp4-internal-registry-operator.jpg)
  ...
  - image stream 使用报错示例：
    ...
    ![imagestream-error-1](pictures/imagestream-error-1.jpg)
    
    ![imagestream-error-2](pictures/imagestream-error-2.jpg)
    
    ![imagestream-error-3](pictures/imagestream-error-3.jpg)
...
- buildconfig（`bc`）、build：
  ...
  - buildconfig 资源中重要的字段说明，如下所示：
    ...
    ![buildconfig-introduce](pictures/buildconfig-introduce.jpg)
  ...
  - buildconfig 中支持的三种构建触发器：
    ...
    - Image change trigger：
      ...
      - S2I 构建镜像的更改将 `自动` 触发构建过程，如下所示：
        ...
        ![imagechange-trigger-auto-build](pictures/imagechange-trigger-auto-build.jpg)
    ...
    - Webhook trigger：
      ...
      - 支持的 webhook 类型：
        ...
        - 1️⃣ Generic：
          ...
          - 💥 `oc start-build` 构建报错示例：
            ...
            ![ocp3-start-build-error-1](pictures/ocp3-start-build-error-1.jpg)
            
            ![ocp3-start-build-error-2](pictures/ocp3-start-build-error-2.jpg)
        ...
        - 2️⃣ GitHub：
          ...
          - 👨‍💻 示例：`Java` 应用 GitHub webhook 配置过程
            ...
            ![buildconfig-webhook-github-5](pictures/buildconfig-webhook-github-5.jpg)
            
            对应用源代码所在的仓库设置 `Webhooks`（只需更改以下两处即可）：
            
            ![buildconfig-webhook-github-1](pictures/buildconfig-webhook-github-1.jpg)
            
            更改代码后提交 commit：
            
            ![buildconfig-webhook-github-2](pictures/buildconfig-webhook-github-2.jpg)
            
            通过 webhook 自动将代码推送至集群，若推送成功无任何报错，推送的事件信息可在 webhook 中查看到详细信息：
            
            ![buildconfig-webhook-github-3](pictures/buildconfig-webhook-github-3.jpg)
            
            根据源代码的更改而自动触发构建：
            
            ![buildconfig-webhook-github-4](pictures/buildconfig-webhook-github-4.jpg)
      ...
      - oc new-app 命令默认创建了 `generic` 和 `github` webhook，这两种自动创建的触发器默认会创建对应的 secret。
        ...
        ![buildconfig-triggers-jq](pictures/buildconfig-triggers-jq.jpg)
  ...
  - 如下所示，构建 `PHP` 应用的 buildconfig 资源定义文件：
    ...
    ![buildconfig-pruning-builds](pictures/buildconfig-pruning-builds.jpg)
  ...
  - 调整构建日志级别的方式：
    ...
    - 方式 1：使用 oc new-app 命令创建应用时指定 `--build-env BUILD_LOGLEVEL=<number>` 选项，即可调整构建日志级别。
      ...
      ![oc-new-app-build-loglevel](pictures/oc-new-app-build-loglevel.jpg)
    ...
    - 如下所示，该环境变量在 `NodeJS` 应用的 buildconfig 资源中的定义：
      ...
      ![buildconfig-build-loglevel](pictures/buildconfig-build-loglevel.jpg)
...
- deploymentconfig（`dc`）、deploy：
  ...
  - 由于 buildconfig 中 imagestreamtag 的改变，deploymentconfig 或 deployment 中可探测到 imagestreamtag 的改变针对新构建应用镜像的自动重新部署。
    ...
    ![ocp3-imagestreamtag-trigger-dc-deploy](pictures/ocp3-imagestreamtag-trigger-dc-deploy.jpg)
    
    👉 OCP 4.x 中 deployment 的 `imagestreamtag trigger`：
    
    ![ocp4-imagestreamtag-trigger-deployment-deploy](pictures/ocp4-imagestreamtag-trigger-deployment-deploy.jpg)
...
- service：
  ...
  - service 的类型：
    ...
    > 
    > ![change-service-type-error](pictures/change-service-type-error.jpg)
  ...
  - service 在 Kubernetes 中的实现方式：
    ...
    - Kubernetes 源码中定义的 service 相关的 iptables 自定义链，如下所示：
      ...
      ![ocp3-network-plugin](pictures/ocp3-network-plugin.jpg)
  ...
  - 🚀 OCP 3.x OVS 网络拓扑示意：
    ...
    ![ocp3-ovs-1](pictures/ocp3-ovs-1.png)
    
    ![ocp3-ovs-2](pictures/ocp3-ovs-2.png)
  
  - 同一节点上 pod 间的通信示意：
    
    ![ocp3-ovs-3](pictures/ocp3-ovs-3.png)
  ...
  - 🚀 OCP 3.x OVS 流表分析示意：
    ...
    ![ocp3-ovs-openflow-1](pictures/ocp3-ovs-openflow-1.jpg)
  ...
  - 🚀 OCP 3.x 中访问使用 `NodePort` service 类型的 pod 跨节点流量分析：
    ...
    ![NodePort-service-iptables-nat-ovs-analyze](pictures/NodePort-service-iptables-nat-ovs-analyze.jpg)
  ...
  - service 的拓展：
    ...
    - 使用原生 `kube-proxy` 实现的 service 与自研的 service 解决方案的响应对比：
      ...
      ![service-performance](pictures/service-performance.jpg)
...
- route：
  ...
  - router 路由原理架构示例：
    ...
    ![ocp3-route-infra](pictures/ocp3-route-infra.jpg)
...
- persistent volume claim（pvc）：  
  ...
  - OCP 部署过程中定义 NFS 存储作为 OCP internal registry 的存储后端：
    ...
    ![ocp3-internal-registry-pvc-1](pictures/ocp3-internal-registry-pvc-1.jpg)
  
  - OCP 集群 default 项目中定义的 pv 与 pvc 的关系：
    
    ![ocp3-internal-registry-pvc-2](pictures/ocp3-internal-registry-pvc-2.jpg)
    
    ![ocp3-internal-registry-pvc-3](pictures/ocp3-internal-registry-pvc-3.jpg)
...
- secret：  
  ...
  - 创建与使用 secret 资源对象：
    ...
    ![oc-create-secret](pictures/oc-create-secret.jpg)
  ...
  - 💎 补充：
    ...
    ![oc-new-app-fail-for-no-secret](pictures/oc-new-app-fail-for-no-secret.jpg)
    
    因此，在项目中创建 secret 并将其链接至名为 default 的 `service account`，实则是为 service account 添加对应的 `imagePullSecrets`，如下所示：
    
    ![oc-secretes-link-help](pictures/oc-secretes-link-help.jpg)
    ...
    ![oc-get-serviceaccounts-default-imagepullsecrets](pictures/oc-get-serviceaccounts-default-imagepullsecrets.jpg)
    
    ![oc-get-secret-quayio](pictures/oc-get-secret-quayio.jpg)
  ...
  - 每个项目中默认的 secret 与 service account（sa） 的关联：
    ...
    - token分别用于：
      ...
      !service-account-secret-1](pictures/service-account-secret-1.jpg)
    
    - pod 运行后将 secret 挂载至 `/var/run/secrets/kubernetes.io/serviceaccount/` 目录中。
    
    - 该目录中的 token 即为 sa 中的 secret 对应的 token。
      
      ![service-account-secret-2](pictures/service-account-secret-2.jpg)
      
      ![service-account-secret-3](pictures/service-account-secret-3.jpg)
...
- configuration map（configmap）： 
  ...
  - 创建与使用 configmap 资源：
    ...
    ![oc-configmap-create](pictures/oc-configmap-create.jpg)
  ...
  - 如下所示，由于注入 configmap 与更改 dc 配置导致两次触发 dc 部署全新的 pod：
    ...
    ![configmap-trigger-dc](pictures/configmap-trigger-dc.jpg)
  ...
  - 💎 补充：
    ...
    ![oc-set-env-or-volume](pictures/oc-set-env-or-volume.jpg)
...
- OCP 资源之间的关系与工作流程：
  ...
  ![ocp3-resource-workflow](pictures/ocp3-resource-workflow.jpg)
...
- 💥 OCP 3.9 版本中删除 route 并重建后无法生效，报错 `HostAlreadyClaimed`，Bugfix 请详见 [Bugfix: route HostAlreadyClaimed](https://bugzilla.redhat.com/show_bug.cgi?id=1660598)。
  ...
  ![ocp3-delete-route-error-1](pictures/ocp3-delete-route-error-1.jpg)
  
  ![ocp3-delete-route-error-2](pictures/ocp3-delete-route-error-2.jpg)
...
- 为 OCP 集群计算节点添加 label 标签：
  ...
  ![node-label](pictures/node-label.jpg)
...
- 管理计算节点的 pod 可调度性：
  ...
  ![node-unscheduleable](pictures/node-unscheduleable.jpg)
  ...
  ![pod-evacuate-1](pictures/pod-evacuate-1.jpg)
  
  ![pod-evacuate-2](pictures/pod-evacuate-2.jpg)
  ...
  ![evacuate-delete-local-data-1](pictures/evacuate-delete-local-data-1.jpg)
  
  ![evacuate-delete-local-data-2](pictures/evacuate-delete-local-data-2.jpg)
...
- OCP 定义的两种策略：
  ...
  - 本地项目策略（local policy）：project-related
    ...
    ![project-level-role](pictures/project-level-role.jpg)
...
- 用户与组可同时绑定一个或多个本地项目角色与集群角色。
  ...
  ![clusterrole-demo](pictures/clusterrole-demo.jpg)
  
  ![verbose-examples](pictures/verbose-examples.jpg)
  ...
  ![self-provisioner-desc](pictures/self-provisioner-desc.jpg)
  ...
  ![serviceaccount-wordpress](pictures/serviceaccount-wordpress.jpg)
