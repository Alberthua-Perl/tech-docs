...
- [DNS 原理与常用配置实现](#dns-原理与常用配置实现)
  - [文档说明](#文档说明)
  - [文档目录](#文档目录)
  - [DNS 域名解析服务之层次结构](#dns-域名解析服务之层次结构)
  - [DNS 域名解析服务之区域](#dns-域名解析服务之区域)
  - [DNS 查询流程](#dns-查询流程)
  - [DNS 资源记录](#dns-资源记录)
  - [DNS 缓存及实现](#dns-缓存及实现)
  - [DNS 域名服务器类型](#dns-域名服务器类型)
  - [BIND DNS 实现权威域名解析服务](#bind-dns-实现权威域名解析服务)
  - [dig 命令使用示例](#dig-命令使用示例)
  - [host 命令使用示例](#host-命令使用示例)
  - [nslookup 命令使用示例](#nslookup-命令使用示例)
  - [DNS 安全问题](#dns-安全问题)
  - [参考链接](#参考链接)
...
- DNS 域名系统层次结构：
  ...
  ![dns-domain-hierarchical-structure](images/dns-domain-hierarchical-structure.jpg)
  
  - 根域名服务器（Root Domain）：用 `·`（句点）表示，根域名服务器全球的数量是固定的，为 `13` 组。
  ...
  - 三级域名服务器或权威域名服务器（如果有）：Third Level Domain 这层严格来说是次级域名的子域，是二层域名派生的域名，通俗说就是网站名，如 `cs.berkeley.edu`。
...
- DNS 查询的两种方式：
  - 1️⃣ 递归查询（`recursive query`）：
    ...
    - 查询过程如下所示：
      ...
      ![dns-recursive-query](images/dns-recursive-query.jpg)
  
  - 2️⃣ 迭代查询（`iterative query`）：
    - 本地 DNS 服务器在收到客户端发起的请求时，并不直接返回查询结果，而是向另一台 DNS 服务器发起请求，以获取查询提示。
    - 查询过程如下所示：

      ![dns-iterative-query](images/dns-iterative-query.jpg)
...
- 🐱‍🏍 DNS 查询的完整示意，如下所示：
  ...
  ![dns-query-progress-1](images/dns-query-progress-1.jpg)
  
  ![dns-query-progress-2](images/dns-query-progress-2.jpg)
  
  ![dns-query-progress-3](images/dns-query-progress-3.jpg)
...
- DNS 资源记录的类型：
  ...
  - <font color=red>**SRV**</font> 资源记录：
    ...
    - 🔥 Kubernetes 与 OpenShift 中 service 的服务发现采用 `SRV` 记录实现。以下示例为 Kubernetes 集群 codeready-workspace 命名空间内的 pod 通过上游 DNS 服务器获得 service 的 SRV 记录的请求查询。
      ...
      ![k8s-pod-dns-resolv-1](images/k8s-pod-dns-resolv-1.png)

      ![k8s-pod-dns-resolv-2](images/k8s-pod-dns-resolv-2.png)
  ...
  - <font color=red>**CNAME**</font> 资源记录（Canonical Name Record）：
    ...
    - 对内部域名与 IP 的变更带来极大的便利
      ...
      ![dns-cname-1](images/dns-cname-1.png)

      ![dns-cname-2](images/dns-cname-2.png)

    - 💥 CNAME 资源记录必须最终解析为具有 A 和/或 AAAA 的记录的名称。

    > 注意：应避免将 CNAME 记录指向其他 CNAME 记录（CNAME 循环）。
  
  - `MX` 资源记录（Mail Exchange Record）：邮件交换记录，将域名映射到接受该域的邮箱服务器域名。
  ...
  - `PTR` 资源记录（Pointer Record）：指针记录，提供 IPv4 或 IPv6 地址到主机名的映射，常用于反向 DNS 解析。
...
- DNS 查询解析涉及的 DNS 缓存：
  - 浏览器缓存：当用户通过浏览器访问某域名时，浏览器首先会在其缓存中查找是否有该域名对应的 IP。
  - 系统缓存：详见下文 "Linux 中 DNS 缓存相关的组件与服务"。
  - 本地 DNS 缓存（如果有）：此处特指硬件服务器，与系统缓存对应。
  - 路由器 router 缓存：一般现在的出口路由器也会设有 DNS 缓存，当以上都查询不到，会在路由器缓存中查询。
  - ISP DNS 缓存：ISP 网络提供的 DNS 服务器缓存。
  - 根域名（root）、顶级域名（TLD）、次级域名（SLD）服务器等缓存：当完成一次完整的查询之后，中间的各 DNS 服务器缓存会将返回的结果一次存入自己的缓存中，以备下次使用提高查询效率。
- DNS 缓存当然能够加快 DNS 查询的效率，但有时也会影响网络问题排查。此时需将 DNS 缓存清除，如下所示：
  - Windows 系统：
    ...
    ![win-dns-cache-1](images/win-dns-cache-1.jpg)
    ...
    ![win-dns-cache-2](images/win-dns-cache-2.jpg)
  ...
  - 对于 Web 浏览器的 DNS 缓存，可在地址栏输入 `chrome://net-internals/#dnsChrome`，然后再点击 `clear host cache`，也可以通过 `Ctrl+Shift+Del` 打开清除浏览器数据窗口进行清除。
    ...
    ![chrome-dns-cache-clear](images/chrome-dns-cache-clear.jpg)
...
- Linux 中 DNS 缓存相关的组件与服务：
  ...
  - `dns_resolver` 内核模块：
    ...
    ![dns_resolver-module](images/dns_resolver-module.png)
...
- nscd 缓存名称服务器使用示例：
  ...
  - nscd 服务配置文件：`/etc/nscd.conf`
    ...
    ![nscd-conf-1](images/nscd-conf-1.jpg)

    ![nscd-conf-2](images/nscd-conf-2.jpg)
  ...
  - 👉 关于 **`/etc/nsswitch.conf`** 配置文件的说明：该文件定义 hosts 域名解析的查询顺序，若 files（/etc/hosts）查询失败，将查询 dns（/etc/resolv.conf）。
    ...
    ![nsswitch-query-sequence-1](images/nsswitch-query-sequence-1.jpg)

    若该文件中定义的查询路径均无法成功返回，但在 nscd hosts 缓存散列表中存在域名与 IP 的对应关系，查询也将成功返回。

    ![nsswitch-query-sequence-2](images/nsswitch-query-sequence-2.jpg)
...
- DNS 技术可提供下面三种类型的服务器：
  - 主服务器（master）：在特定区域内具有唯一性，负责维护该区域内的域名与 IP 地址之间的对应关系。
  - 从服务器（slave）：从主服务器中获得域名与 IP 地址的对应关系并进行维护，以防主服务器宕机等情况。
  - 缓存服务器（cache）：通过向其他域名解析服务器查询获得域名与 IP 地址的对应关系，并将经常查询的域名信息保存到服务器本地，以此来提高重复查询时的效率。
...
- bind 服务安装与配置：
  - 各服务器节点角色分布：
    ...
    ![named-service-nodes](images/named-service-nodes.jpg)
  
  - 各服务器节点物理拓扑示意：

    ![named-service-arch](images/named-service-arch.jpg)
...
- **主 DNS 权威服务器部署：**
  ...
  - `/etc/named.conf` 主配置文件：包含 `options` 段、`logging` 段、`zone` 段、`include` 指令，还可包含可选的 `view` 段。 
  ...
  ![named-conf-demo](images/named-conf-demo.jpg)

  通过配置主 DNS 服务器 /etc/named.conf 文件来实现当主 DNS 服务器上的区域发生更改时通知所有从 DNS 服务器进行区域更新。
  
  ![named-conf-notify](images/named-conf-notify.jpg)
  ...
  ![named-zone-conf](images/named-zone-conf.png)
  ...
  ![var-named-zone-data-1](images/var-named-zone-data-1.jpg)
  
  ```bash
  $ sudo vim /var/named/lab.example.com.zone
  ```
  
  ![var-named-zone-data-2](images/var-named-zone-data-2.jpg)
  
  ```bash
  $ sudo vim /var/named/10.197.11.arpa
  ```
  
  ![var-named-zone-data-3](images/var-named-zone-data-3.jpg)
...
- **从 DNS 权威服务器部署：**
  ...
  ![named-conf-slave](images/named-conf-slave.png)
  ...
  ![named-zone-conf-slave](images/named-zone-conf-slave.png)
...
- **安全加密传输：**
  ...
  - 配置主 DNS 服务器加密传输：
    ...
    ![named-master-sectrans-1](images/named-master-sectrans-1.png)
    ...
    ![named-master-sectrans-2](images/named-master-sectrans-2.png)
    ...
    ![named-master-sectrans-3](images/named-master-sectrans-3.png)
    ...
    ![named-master-sectrans-4](images/named-master-sectrans-4.png)

    ![named-master-sectrans-5](images/named-master-sectrans-5.png)
  ...
  - 配置从 DNS 服务器加密传输：
    ...
    ![named-slave-sectrans-1](images/named-slave-sectrans-1.png)
    ...
    ![named-slave-sectrans-2](images/named-slave-sectrans-2.png)
...
- 验证 DNS 解析服务：
  - 使用 `dig` 命令验证DNS正向解析结果：
    ...
    ![dig-test-1](images/dig-test-1.png)

    ![dig-test-2](images/dig-test-2.png)
  
  - 使用 `dig -x` 命令验证 DNS 反向解析结果。
  - 也可使用 `nslookup` 命令验证 DNS 正向与反向解析结果。

    ![nslookup-test](images/nslookup-test.jpg)
...
- dig 命令输出六段信息，如下所示：
  ...
  ![dig-usage-1](images/dig-usage-1.png)
  
  status 状态通常包括：
  
  - `NOERROR` 状态：表示已成功解析查询
  - `SERVFAIL` 状态：常见原因是 DNS 服务器无法与权威名称服务器进行通信。可能是由于权威名称服务器不可用。还可能是由于网络问题干扰了 DNS 服务器与权威名称服务器之间的通信。
  - `NXDOMAIN` 状态：
...
- 常用命令示例如下所示：
  ...
  ![dig-usage-2](images/dig-usage-2.png)
  ...
  ![dig-usage-3](images/dig-usage-3.png)
  ...
  ![dig-usage-4](images/dig-usage-4.png)
...
- 如下所示：
  ...
  ![host-usage](images/host-usage.png)
...
- 交互模式，如下所示：
  ...
  ![nslookup-usage-1](images/nslookup-usage-1.png)
  
  ![nslookup-usage-2](images/nslookup-usage-2.png)
  
  ![nslookup-usage-3](images/nslookup-usage-3.png)
...
- 非交互模式，如下所示：
  ...
  ![nslookup-usage-4](images/nslookup-usage-4.png)
  ...
  ![nslookup-usage-5](images/nslookup-usage-5.png)
  ...
  ![nslookup-usage-6](images/nslookup-usage-6.png)

## DNS 安全问题

- 域名抢注：有些黄牛会批量抢注大量域名，然后转身高价卖给那些对域名感兴趣的人，谋取暴利。
- DNS 缓存污染：中间 DNS 服务器将上级服务器返回结果进行修改，给客户端一个改变了（污染）的信息。
- DNS 劫持：
  - 与 DNS 缓存污染比较类似，不过这种可以被外部第三者劫持进行恶意修改。
  - 有些恶意的域名服务器故意更改一些域名的解析结果，将用户引向一个错误的目标地址，这种称为 DNS 劫持，主要用来阻止用户访问某些特定的网站，或者是将用户引导到广告页面。
  - 如下图所示：

    ![dns-hijack](images/dns-hijack.png)
...
- DNS 欺骗：
  ...
  - 如下图所示：
    ...
    ![dns-cheat](images/dns-cheat.png)
  
  - 通常可配置 bind、unbound 缓存名称服务器的 DNSSEC 验证预防 DNS 欺骗的发生。

- DNS 放大攻击：一种 `DDoS` 攻击，利用 DNS 回复包比请求包大的特点，放大流量，伪造请求包的源 IP 地址为受害者 IP，将应答包的流量引入受害者的服务器。
