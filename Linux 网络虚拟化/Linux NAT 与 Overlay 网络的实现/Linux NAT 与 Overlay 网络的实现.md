...
- [🐧 Linux NAT 与 Overlay 网络的实现](#-linux-nat-与-overlay-网络的实现)
  - [文档说明](#文档说明)
  - [文档目录](#文档目录)
  - [Linux kernel 原生支持的网络技术](#linux-kernel-原生支持的网络技术)
  - [iptables NAT 表的 DNAT、SNAT 与 MASQUERADE](#iptables-nat-表的-dnatsnat-与-masquerade)
  - [IPIP 隧道原理与 Linux 的原生实现](#ipip-隧道原理与-linux-的原生实现)
  - [GRE 隧道原理与 Linux 的原生实现](#gre-隧道原理与-linux-的原生实现)
  - [VXLAN 隧道原理与 Linux 的原生实现](#vxlan-隧道原理与-linux-的原生实现)
  - [参考链接](#参考链接)
...
- Linux kernel 原生支持的隧道技术（Overlay 网络）：
  ...
  >
  >    ![man-ip-link](images/man-ip-link.png)
  >
...
- Linux 主机作为 NAT 网关设备实现 `DNAT`：
  ...
  - `net.ipv4.ip_forward` 内核参数作用于 `FORWARD` 链，POSTROUTING 链定义数据包从网口发出的规则。
    ...
    ![linux-nat-gateway-dnat](images/linux-nat-gateway-dnat.jpg)
  ...
  - 客户端访问应用测试，该应用为提供 docker 官方文档的 nginx 容器（外部 -> 内部）。
    ...
    ![linux-nat-gateway-dnat-client](images/linux-nat-gateway-dnat-client.jpg)

- Linux 主机作为 NAT 网关设备实现 SNAT：
  - 即，从内部客户端访问外部网络。
  - 可使用 SNAT 将 Linux 主机作为连接外部网络与内部网络的网关设备。

    ![linux-nat-gateway-snat](images/linux-nat-gateway-snat.jpg)
  ...
  - 客户端将该 NAT 设备作为默认网关即可对外访问（内部 -> 外部）。
    ...
    ![linux-nat-gateway-snat-client](images/linux-nat-gateway-snat-client.jpg)
...
- iptables 各个链（chain）与表（table）的关系：**四表五链**
  ...
  ![iptables-for-all](images/iptables-for-all.jpg)
...
- IPIP 隧道在 Linux 上的实现：
  - IPIP 隧道实验网络拓扑：
    ...
    <img src="images/ipip-tunnel.png" style="zoom:80%;" />

  - docker-arch 与 podman-arch 作为两端内部网络中的主机。
  - bastion0 与 bastion1 作为两端内部网络的路由器网关。
  - 两端内部网络中的主机设置对应的路由器网关，如下所示：
  
    ![docker-arch-host-route-table](images/docker-arch-host-route-table.png)
  ...
  - IPIP 隧道创建：
    ...
    ![ipip-tunnel-demo-1](images/ipip-tunnel-demo-1.jpg)
  
  - 在 docker-arch 主机上对 podman-arch 持续发送 ICMP 数据包，使用 tcpdump 对 bastion1 的 eth1 抓包，查看 IPIP 数据包信息。
  
    ![ipip-tunnel-demo-2](images/ipip-tunnel-demo-2.jpg)
...
- GRE 隧道简介：
  ...
  - GRE 是一种 `IP-in-IP` 的隧道，可以对某些网络层协议的数据报进行封装，使这些被封装的数据报能够在IPv4/IPv6 网络中传输。
    ...
    ![gre-tunnel](images/gre-tunnel.jpg)
...
- GRE 隧道在 Linux 上的实现：
  - GRE 隧道实现拓扑示意：
    ...
    ![gre-tunnel-demo-1](images/gre-tunnel-demo-1.jpg)

  - 两端内部网络中主机网络信息：docker-arch 主机、podman-arch 主机

    ![gre-tunnel-demo-2](images/gre-tunnel-demo-2.jpg)

    ![gre-tunnel-demo-3](images/gre-tunnel-demo-3.jpg)
  ...
  - R1 与 R2 路由器的 GRE 隧道与路由表信息：
    ...
    ![gre-tunnel-demo-4](images/gre-tunnel-demo-4.jpg)

    ![gre-tunnel-demo-5](images/gre-tunnel-demo-5.jpg)

  - 由于 GRE 隧道建立在路由器之间，因此在 docker-arch 主机上持续发送 ICMP 包，使用 tcpdump 在 R2 路由器上抓包，查看 GRE 数据包信息。

    ![gre-tunnel-demo-6](images/gre-tunnel-demo-6.jpg)
...
- 需要 VXLAN 的原因：
  ...
  - 多租户场景：IP 地址重叠
- VXLAN 协议报文简介：
  - VXLAN 报文：虚拟可扩展局域网（VXLAN，virtual extensible LAN）是一种 overlay 网络技术，使用 `MAC-in-UDP` 的方法进行封装，共 `50` 字节的封装报文头。
  - 报文格式如下所示：

    ![vxlan-packet](images/vxlan-packet.png)

    - VXLAN 报文头（VXLAN Header）：共计 8 个字节（byte），目前使用的是 Flags 中的一个 8 bit 的标识位和 24 bit 的 `VNI`（VXLAN network identifier），其余部分没有定义，但是在使用的时候必须设置为 0x0000。
...
- VXLAN ARP 请求与应答：
  ...
  - ARP 应答（response）：
    ...
    ![vxlan-arp-response](images/vxlan-arp-response.png)
...
- VXLAN 数据传输过程：
  ...
  - 整个 VXLAN 相关的行为（可能穿越多个网关），对虚拟机而言是透明的，虚拟机不会感受传输的过程。
    ...
    ![vxlan-communication-progress](images/vxlan-communication-progress.png)
...
- VXLAN 网关（VXLAN gateway）：
  ...
  ![vxlan-gateway](images/vxlan-gateway.png)
...
- VXLAN 部署：
  - 纯 VXLAN 部署场景：对于连接到 VXLAN 内的虚拟机，由于虚拟机的 VLAN 信息不再作为转发的依据，虚拟机的迁移也就不再受三层网关的限制，可以实现跨越三层网关的迁移。
  - VXLAN 与 VLAN 混合部署：
...
- VXLAN 隧道在 Linux 上的实现：
  - 以单播模式创建 Linux 原生 VXLAN 隧道：
    - 实现的网络拓扑：
      ...
      <img src="images/vxlan-singlecast-demo-1.jpg">
    ...
    - 实现命令如下所示：
      ...
      ![vxlan-singlecast-demo-2](images/vxlan-singlecast-demo-2.jpg)
      ...
      >
      >    <img src="images/vxlan-singlecast-demo-3.jpg">
      >
    ...
    - 两端 VXLAN 隧道配置完成后，在 VM1 上持续发送 ICMP 包，并在 VM2 上使用 tcpdump 抓取相应网络接口的数据包，如下所示：
      ...
      ![vxlan-singlecast-demo-4](images/vxlan-singlecast-demo-4.jpg)
  ...
  - 以多播模式创建 Linux 原生 VXLAN 隧道：
    ...
    - 实现的网络拓扑：
      ...
      <img src="images/vxlan-multicast-demo-1.png">
    ...
    - 两端 VXLAN 隧道配置完成后，在 VM1 上持续向 VM2 发送 ICMP 包，并在 VM2 上使用 tcpdump 抓取 eth1与 vxlan100 网络接口的数据包，如下所示：
      - 通过 eth1 的数据包使用 VXLAN 隧道通信，数据包以 UDP 的方式发送。
        ...
        ![vxlan-multicast-demo-2](images/vxlan-multicast-demo-2.png)
  
      - 通过 vxlan100 的数据包去除了 VXLAN 报文头，只具有原始的 ICMP 报文。
  
        ![vxlan-multicast-demo-3](images/vxlan-multicast-demo-3.png)
    ...
    - 首次 ping 对端 veth0 的 IP 地址（内部 IP 地址）时，需进行 ARP 广播以获取对端 veth0 的 MAC 地址，并且该 ARP 广播包需通过 IGMP 发送至多播地址（239.1.1.1）。
      ...
      ![vxlan-multicast-demo-4](images/vxlan-multicast-demo-4.png)
  
      ![vxlan-multicast-demo-5](images/vxlan-multicast-demo-5.png)
  
    - UDP 数据包转换为 VXLAN 封装的数据包后，可直接查看 VXLAN 所封装的内层原始载荷数据，如 ARP 请求与ICMP 报文。
      - ARP 请求报文结构：目的地址为多播组地址
  
        ![vxlan-multicast-demo-6](images/vxlan-multicast-demo-6.png)
  
      - ICMP 报文结构：目的地址为单播地址
  
        ![vxlan-multicast-demo-7](images/vxlan-multicast-demo-7.png)
    ...
    - VM1 与 VM2 通信建立后，可分别查看 `fdb` 表项与 `ARP` 缓存：
      ...
      ![vxlan-multicast-demo-8](images/vxlan-multicast-demo-8.png)

## 参考链接

- [NAT 学习（DNAT与SNAT）](https://blog.csdn.net/nmjhehe/article/details/85475310)
- [iptables 中 DNAT、SNAT 与 MASQUERADE 的理解](https://www.cnblogs.com/fyc119/p/7489052.html)
- [Linux IPIP 隧道及其实现](https://www.cnblogs.com/weifeng1463/p/6805856.html)
- [GRE Tunneling](https://tldp.org/HOWTO/Adv-Routing-HOWTO/lartc.tunnel.gre.html)
- [VXLAN 原理](https://www.cnblogs.com/wipan/p/9220615.html)
- [VXLAN 协议](https://www.cnblogs.com/wipan/p/9220615.html)
- [Linux 上实现 VXLAN 网络：蚂蚁金服](https://cizixs.com/2017/09/28/linux-vxlan/)
