...
- 什么是 System V IPC 机制中的信号集和信号量？信号量主要解决两类问题：
  - 互斥（Mutual Exclusion）：信号量初值设为 1，同一时刻只允许一个进程进入临界区，这种信号量称为二进制信号量（binary semaphore），效果类似一把锁。
  - 同步（Synchronization）：信号量初值设为 N，控制对有限资源的并发访问数量，如"最多允许 3 个进程同时读数据库连接池"。
- 什么是临界区和竞态条件？
  - 临界区（Critical Section）：临界区是指访问共享资源（临界资源）的那段代码 —— 同一时刻只允许一个进程（或线程）执行它，否则会产生数据错乱。
  - System V IPC 实际是三套机制：
    - 共享内存（Shared Memory）：速度最快的进程间数据交换（直接读写同一块内存）
    - 消息队列：带格式的、有边界的数据传递（还内置了消息类型过滤）
    - 信号量：不管数据，只管"什么时候能访问"
    - 三者的常见组合是：共享内存传数据 + 信号量管同步（因为共享内存本身完全没有同步能力，必须配信号量）；或者消息队列传数据 + 信号量管阻塞等待。
...
- **CIB 关注的性能问题：**
  - 如何排查定位 CPU 持续高负载或高利用率？计算密集+IO 密集
  - 如何排查定位 IO wait 高于阈值的场景？IO 密集
  - 如何排查定位网络持续丢包或间歇性丢包导致的性能问题？
  - 可能涉及的场景：
    - 磁盘 IO 写延迟（数据库）
    - 数据库语句优化
    - 应用链路追踪：前端+缓存+MQ+后端+分布式锁
    - 网络设备：ring buffer 过小，缓冲不够
    - TCP 层读写缓存不够：rmem+wmem
      - rmem 不足：
      - 数据来的太快：iface 侧 ring buffer 不足
      - 数据走的太晚：user space 侧来不及处理
      - sk->lock 锁竞争+自旋锁竞争导致的连接超时
